...
3. Test changes as appropriate (run `cargo build --workspace`, `cargo test --workspace`)
...
- 3.1 Update `crates/api/src/menu.rs`:
  ...
  - Rewrite `create_menu()` to:
    ...
    2. For each group, collect matching `MENU_ITEMS` and sort by priority
    3. Build `MenuItem::group()` with nested `MenuItem::item()` calls
    4. Return `MenuState::new(groups)`
...
2. **Tight coupling**: Menu items should reference commands by name, not function pointers
3. **Ignoring existing patterns**: Study `action!`, `command!` macros before implementing
4. **Monolithic files**: Split menu definitions by group (file.rs, edit.rs, etc.)
5. **Missing priority system**: Always include priority for predictable ordering
...
3. Menu displays identically to current behavior
4. Extensions can add menu items by using `menu_group!` and `menu_item!` macros
5. Code follows existing patterns (compare with `action!`, `command!` implementations)
