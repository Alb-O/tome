...
2. Run `cargo check --workspace` after each major change
3. Run `cargo test --workspace` after completing the migration
4. Do not stop at partial completion - migrate ALL motion references
5. If you encounter architectural blockers, document them and propose solutions
...
2. A public `MotionKey` constant for internal use
...
2. Define `MotionKey` type alias
3. Implement `Key::new()`, `Key::def()`, `Key::name()` (delegates to def)
4. Verify: `cargo check -p evildoer-registry-motions`
...
2. Handle the static visibility (needs to be accessible for Key::new)
3. Ensure distributed slice registration still works
4. Verify: `cargo check -p evildoer-registry-motions`
...
2. Verify all motion definitions generate their keys
3. Verify: `cargo check -p evildoer-registry-motions`
...
2. Change `selection_motion` similarly
3. Change `insert_with_motion` similarly
4. Update internal lookup logic (now just `motion.def()`)
5. Verify: `cargo check -p evildoer-registry-actions`
...
2. Replace all `cursor_motion(ctx, "left")` with `cursor_motion(ctx, motions::left)`
3. Replace all `selection_motion(ctx, "...")` similarly
4. Replace all `insert_with_motion(ctx, "...")` similarly
...
2. Update all callers (likely in core/api crates)
...
3. Verify no string-based motion lookups remain in internal code (only at boundaries)
...
2. **Duplicate motion names**: If two crates define same motion name, keys would conflict - document this limitation
3. **Test code**: Tests that verify runtime lookup should continue using strings
4. **Scroll actions**: `crates/registry/actions/src/impls/scroll.rs` may use motions differently - check
...
2. **Don't break incrementally**: Each phase should leave the build working
3. **Don't over-engineer**: Key<T> is just a pointer wrapper, keep it simple
4. **Don't forget Debug**: Keys should be debuggable (print the motion name)
