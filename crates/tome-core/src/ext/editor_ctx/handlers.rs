...
fn run_handlers(handlers: &[ResultHandler], result: &ActionResult, ctx: &mut EditorContext, extend: bool) -> bool {
...
pub fn dispatch_result(result: &ActionResult, ctx: &mut EditorContext, extend: bool) -> bool {
    match result {
        ...
        ActionResult::ModeChange(_) => run_handlers(&RESULT_MODE_CHANGE_HANDLERS, result, ctx, extend),
        ActionResult::CursorMove(_) => run_handlers(&RESULT_CURSOR_MOVE_HANDLERS, result, ctx, extend),
        ActionResult::Motion(_) => run_handlers(&RESULT_MOTION_HANDLERS, result, ctx, extend),
        ActionResult::InsertWithMotion(_) => run_handlers(&RESULT_INSERT_WITH_MOTION_HANDLERS, result, ctx, extend),
        ActionResult::Edit(_) => run_handlers(&RESULT_EDIT_HANDLERS, result, ctx, extend),
        ActionResult::Quit | ActionResult::ForceQuit => run_handlers(&RESULT_QUIT_HANDLERS, result, ctx, extend),
        ...
        ActionResult::SearchNext { .. } => run_handlers(&RESULT_SEARCH_NEXT_HANDLERS, result, ctx, extend),
        ActionResult::SearchPrev { .. } => run_handlers(&RESULT_SEARCH_PREV_HANDLERS, result, ctx, extend),
        ActionResult::UseSelectionAsSearch => run_handlers(&RESULT_USE_SELECTION_SEARCH_HANDLERS, result, ctx, extend),
        ActionResult::SplitLines => run_handlers(&RESULT_SPLIT_LINES_HANDLERS, result, ctx, extend),
        ActionResult::JumpForward => run_handlers(&RESULT_JUMP_FORWARD_HANDLERS, result, ctx, extend),
        ActionResult::JumpBackward => run_handlers(&RESULT_JUMP_BACKWARD_HANDLERS, result, ctx, extend),
        ActionResult::SaveJump => run_handlers(&RESULT_SAVE_JUMP_HANDLERS, result, ctx, extend),
        ActionResult::RecordMacro => run_handlers(&RESULT_RECORD_MACRO_HANDLERS, result, ctx, extend),
        ActionResult::PlayMacro => run_handlers(&RESULT_PLAY_MACRO_HANDLERS, result, ctx, extend),
        ActionResult::SaveSelections => run_handlers(&RESULT_SAVE_SELECTIONS_HANDLERS, result, ctx, extend),
        ActionResult::RestoreSelections => run_handlers(&RESULT_RESTORE_SELECTIONS_HANDLERS, result, ctx, extend),
        ActionResult::ForceRedraw => run_handlers(&RESULT_FORCE_REDRAW_HANDLERS, result, ctx, extend),
        ActionResult::RepeatLastInsert => run_handlers(&RESULT_REPEAT_LAST_INSERT_HANDLERS, result, ctx, extend),
        ActionResult::RepeatLastObject => run_handlers(&RESULT_REPEAT_LAST_OBJECT_HANDLERS, result, ctx, extend),
        ActionResult::DuplicateSelectionsDown => run_handlers(&RESULT_DUPLICATE_SELECTIONS_DOWN_HANDLERS, result, ctx, extend),
        ActionResult::DuplicateSelectionsUp => run_handlers(&RESULT_DUPLICATE_SELECTIONS_UP_HANDLERS, result, ctx, extend),
        ActionResult::MergeSelections => run_handlers(&RESULT_MERGE_SELECTIONS_HANDLERS, result, ctx, extend),
        ...
        ActionResult::TabsToSpaces => run_handlers(&RESULT_TABS_TO_SPACES_HANDLERS, result, ctx, extend),
        ActionResult::SpacesToTabs => run_handlers(&RESULT_SPACES_TO_TABS_HANDLERS, result, ctx, extend),
        ActionResult::TrimSelections => run_handlers(&RESULT_TRIM_SELECTIONS_HANDLERS, result, ctx, extend),
        ActionResult::OpenScratch { .. } => run_handlers(&RESULT_OPEN_SCRATCH_HANDLERS, result, ctx, extend),
        ActionResult::CloseScratch => run_handlers(&RESULT_CLOSE_SCRATCH_HANDLERS, result, ctx, extend),
        ActionResult::ToggleScratch => run_handlers(&RESULT_TOGGLE_SCRATCH_HANDLERS, result, ctx, extend),
        ActionResult::ExecuteScratch => run_handlers(&RESULT_EXECUTE_SCRATCH_HANDLERS, result, ctx, extend),
    }
}
...
mod tests {
    ...
    fn test_handler_coverage_counts() {
        ...
        assert!(total >= 8, "expected handlers registered for major variants");
    }
}
