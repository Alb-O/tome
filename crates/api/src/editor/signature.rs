...
impl SignatureHelpState {
    ...
    pub fn parameter_index(&self) -> Option<usize> {
        match self {
            Self::Inactive => None,
            Self::Requesting { parameter_index, .. } => Some(*parameter_index),
            Self::Active { parameter_index, .. } => Some(*parameter_index),
        }
    }
    ...
    pub fn advance_parameter(&mut self) {
        match self {
            Self::Requesting { parameter_index, .. } | Self::Active { parameter_index, .. } => {
                *parameter_index += 1;
            }
            _ => {}
        }
