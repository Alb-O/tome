...
pub fn get_client_for_buffer(registry: &Registry, buffer: &Buffer) -> Option<ClientHandle> {
...
pub async fn request_hover(registry: &Registry, buffer: &Buffer) -> Option<Hover> {
...
pub async fn request_signature_help(registry: &Registry, buffer: &Buffer) -> Option<SignatureHelp> {
	let client = get_client_for_buffer(registry, buffer)?;
	...
	client
		.signature_help(uri, position, None)
		.await
		.ok()
		.flatten()
}
...
pub async fn request_code_actions(
	...
	client
		.code_action(uri, lsp_range, context)
		.await
		.ok()
		.flatten()
}
...
pub fn get_selected_code_action(ui: &UiManager) -> Option<CodeActionResult> {
	let popup = ui
		.popups
		.get_popup::<CodeActionsPopup>("lsp-code-actions")?;
	...
	let location = match response {
		...
		GotoDefinitionResponse::Link(links) => {
			links
				.into_iter()
				.next()
				.map(|l| xeno_lsp::lsp_types::Location {
					uri: l.target_uri,
