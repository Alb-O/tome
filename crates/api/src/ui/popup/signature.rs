    ...
    fn find_parameter_range(&self, sig: &SignatureInformation) -> Option<(usize, usize)> {
        ...
        match &param.label {
            ParameterLabel::Simple(name) => {
                ...
                sig.label.find(name.as_str()).map(|start| (start, start + name.len()))
            }
...
mod tests {
    ...
    fn test_signature_popup_creation() {
        let help = SignatureHelp {
            signatures: vec![make_signature("fn foo(a: i32, b: String)", vec!["a: i32", "b: String"])],
    ...
    fn test_cycle_signatures() {
        let help = SignatureHelp {
            signatures: vec![
                ...
                make_signature("fn foo(a: i32, b: i32, c: i32)", vec!["a: i32", "b: i32", "c: i32"]),
            ],
            active_signature: Some(0),
            active_parameter: None,
        };
