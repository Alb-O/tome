...
#[cfg(feature = "lsp")]
use super::{CodeActionResult, CompletionAcceptResult};
...
impl PopupManager {
	...
	pub fn update_completion_filter(&mut self, filter_text: String) -> bool {
		...
		let popup = self.popups.iter_mut().find(|p| p.id() == "lsp-completion");
	...
	pub fn should_dismiss_completion(&self) -> bool {
		let popup = self.popups.iter().find(|p| p.id() == "lsp-completion");
	...
	pub fn try_accept_location_picker(
		...
		self.popups.remove(popup_idx);
		self.rendered_areas
			.retain(|(id, _)| id != "lsp-location-picker");
	...
	#[cfg(feature = "lsp")]
	pub fn try_accept_code_actions(&mut self, key: &KeyEvent) -> Option<CodeActionResult> {
		if !matches!(key.code, KeyCode::Enter) {
			return None;
		}
		...
		let popup_idx = self
			.popups
			.iter()
			.position(|p| p.id() == "lsp-code-actions")?;

		let result = {
			let popup = &self.popups[popup_idx];
			if let Some(actions_popup) = popup.as_any().downcast_ref::<super::CodeActionsPopup>() {
				actions_popup.accept_selected()
			} else {
				None
			}
		};
		...
		self.popups.remove(popup_idx);
		self.rendered_areas
			.retain(|(id, _)| id != "lsp-code-actions");
