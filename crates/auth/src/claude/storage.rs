...
pub fn save_auth(data_dir: &Path, state: &AuthState) -> AuthResult<()> {
    fs::create_dir_all(data_dir).map_err(|e| {
        AuthError::Storage(format!("failed to create {}: {e}", data_dir.display()))
    })?;

    let path = auth_file_path(data_dir);
    let contents = serde_json::to_string_pretty(state).map_err(|e| {
        AuthError::Storage(format!("failed to serialize: {e}"))
    })?;
    ...
    #[cfg(unix)]
    {
        ...
        fs::set_permissions(&temp_path, perms).map_err(|e| {
            AuthError::Storage(format!("failed to set permissions: {e}"))
        })?;
    }

    fs::rename(&temp_path, &path).map_err(|e| {
        AuthError::Storage(format!("failed to rename: {e}"))
    })?;
...
mod tests {
    ...
    fn save_and_load_oauth() {
        let temp = TempDir::new().unwrap();
        let state = AuthState::from_oauth(
            "access".into(),
            "refresh".into(),
            1234567890000,
        );
