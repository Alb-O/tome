...
impl ToastState {
    ...
    fn update(&mut self, delta: Duration) {
        ...
        if !phase_duration.is_zero()
            && matches!(self.phase, AnimationPhase::Entering | AnimationPhase::Exiting)
        {
...
fn calculate_toast_size(toast: &Toast, area: Rect) -> (u16, u16) {
    ...
    let width = (content_width + 2 + padding_h).max(3).min(max_width);
...
impl ToastManager {
    ...
    fn render_anchor_group(&mut self, anchor: Anchor, ids: &[u64], area: Rect, buf: &mut Buffer) {
        ...
        sorted_ids.sort_by_key(|id| {
            self.states.get(id).map(|s| s.created_at).unwrap_or(Instant::now())
        });
